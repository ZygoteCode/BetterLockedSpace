...
public class LockedSpaceManager
{
    ...
    public static void CreateLockedSpace(string spaceName, string password)
    {
        ...
        {
            MessageBox.Show("The locked space name is empty.", "BetterLockedSpace", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return;
        }

        if (File.Exists($"output_spaces\\{spaceName}.lockedspace"))
        {
            MessageBox.Show("The specified locked space name already exists in the folder 'output_spaces'.", "BetterLockedSpace", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return;
        }

        if (password.Replace(" ", "").Replace('\t'.ToString(), "") == "")
        {
            MessageBox.Show("The password is empty.", "BetterLockedSpace", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return;
        }
        ...
        if (paths.Length == 0)
        {
            MessageBox.Show("No files/folders to include in the space are available in the 'space_to_crypt' folder.", "BetterLockedSpace", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return;
        }
        ...
        File.WriteAllBytes($"output_spaces\\{spaceName}.lockedspace", 
            LockedSpaceCrypto.Encrypt(fullBytes, Encoding.Unicode.GetBytes(password)));

        MessageBox.Show("Succesfully created your locked space! The output locked space is in the folder 'output_spaces'.", "BetterLockedSpace", MessageBoxButtons.OK, MessageBoxIcon.Information);
    }
    ...
    public static void DecryptLockedSpace(string spacePath, string password)
    {
        ...
        {
            MessageBox.Show("The specified locked space path does not exist.", "BetterLockedSpace", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return;
        }

        if (!Path.GetExtension(spacePath).Equals(".lockedspace"))
        {
            MessageBox.Show("The extension of the specified locked space path is not valid.", "BetterLockedSpace", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return;
        }
        ...
        byte[] decryptedBytes = LockedSpaceCrypto.Decrypt(File.ReadAllBytes(spacePath), Encoding.Unicode.GetBytes(password));
        ...
        MessageBox.Show("Succesfully decrypted your locked space! The output files/folders are in the folder 'decrypted_space'.", "BetterLockedSpace", MessageBoxButtons.OK, MessageBoxIcon.Information);
    }
